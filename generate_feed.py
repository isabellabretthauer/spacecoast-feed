...
def scrape_launches():
    ...
    for block in soup.select("div.card__content"):
        ...
        # grab the big launch image from the <figure> if present
...
def scrape_events():
    ...
    for title_tag in soup.find_all("h4"):
        ...
        img_tag = title_tag.find_next("img.attachment-post-thumbnail") or title_tag.find_next("img.wp-post-image")
        img_url = img_tag.get("data-lazy-src") or img_tag.get("src") if img_tag else None

        # split description if any
        desc_el = title_tag.find_next("p")
